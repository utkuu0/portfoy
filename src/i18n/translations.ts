export const translations = {
  tr: {
    nav: {
      about: "Hakkımda",
      projects: "Projeler",
      skills: "Yetenekler",
      contact: "İletişim"
    },
    hero: {
      greeting: "Merhaba, ben",
      role: "Full Stack & Mobile Developer",
      description: "Modern web ve mobil uygulamalar geliştiriyorum. Kullanıcı deneyimi odaklı, performanslı ve ölçeklenebilir çözümler sunuyorum."
    },
    about: {
      title: "Hakkımda",
      p1: "Yazılım geliştirme tutkusuyla dolu bir geliştiriciyim. Android uygulamalarından web projelerine kadar geniş bir yelpazede çalışıyorum.",
      p2: "Modern teknolojileri kullanarak kullanıcı dostu ve performanslı uygulamalar geliştirmeyi seviyorum. Sürekli öğrenme ve kendimi geliştirme konusunda tutkuluyum.",
      stats: {
        experience: "Yıl Deneyim",
        projects: "Proje",
        clients: "Mutlu Müşteri"
      }
    },
    projects: {
      title: "Projeler",
      subtitle: "Üzerinde çalıştığım ve geliştirdiğim projelerden bazıları",
      view_projects: "Projelerimi Gör" // CTA if needed
    },
    skills: {
      title: "Yetenekler",
      subtitle: "Kullandığım teknolojiler ve araçlar"
    },
    contact: {
      title: "İletişime Geçin",
      subtitle: "Bir proje fikriniz mi var? Birlikte çalışmak ister misiniz? Bana ulaşın!",
      form: {
        name: "Adınız",
        email: "E-posta adresiniz",
        subject: "Konu",
        message: "Mesajınız...",
        submit: "Mesaj Gönder"
      }
    },
    footer: {
      tagline: "Yazılım ile fikirler hayat buluyor.",
      rights: "Tüm hakları saklıdır."
    }
  },
  en: {
    nav: {
      about: "About",
      projects: "Projects",
      skills: "Skills",
      contact: "Contact"
    },
    hero: {
      greeting: "Hello, I'm",
      role: "Full Stack & Mobile Developer",
      description: "I build modern web and mobile applications. I provide user experience focused, performant and scalable solutions."
    },
    about: {
      title: "About Me",
      p1: "I am a developer full of passion for software development. I work in a wide range from Android applications to web projects.",
      p2: "I love developing user-friendly and performant applications using modern technologies. I am passionate about continuous learning and self-improvement.",
      stats: {
        experience: "Years Experience",
        projects: "Projects",
        clients: "Happy Clients"
      }
    },
    projects: {
      title: "Projects",
      subtitle: "Some of the projects I have worked on and developed",
      view_projects: "View Projects"
    },
    skills: {
      title: "Skills",
      subtitle: "Technologies and tools I use"
    },
    contact: {
      title: "Get in Touch",
      subtitle: "Have a project idea? Want to work together? Contact me!",
      form: {
        name: "Your Name",
        email: "Your Email",
        subject: "Subject",
        message: "Your Message...",
        submit: "Send Message"
      }
    },
    footer: {
      tagline: "Software brings ideas to life.",
      rights: "All rights reserved."
    }
  }
};
